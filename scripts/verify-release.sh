...
find_python() {
  local candidates=()
  if [[ -n "${CIVICPROCURE_RELEASE_PYTHON:-}" ]]; then
    candidates+=("${CIVICPROCURE_RELEASE_PYTHON}")
  fi
...
exec((root / "civicprocure" / "__init__.py").read_text(encoding="utf-8"), namespace)
...
for path in [
    ...
    assert "1.0.0.dev0" not in text, f"stale dev version in {path}"
...
dist = Path("dist")
wheel = dist / "civicprocure-0.2.0-py3-none-any.whl"
sdist = dist / "civicprocure-0.2.0.tar.gz"
